namespace AOC
{
    ...
    {
        public static string Raw { get; } = @"abaaacccccccccaaaaaaccccccccccccccccaacccccccccccaacaaaaaaaaaaaaaaaaaccaaaaacccaaaaccccccccccccccccccccccccccccccccccccccccccccccccccccccccaaaaa
...
abaaaccccccccccccaaaaaaaaaacccccccccaaaaaaaacccccccccccaaaaaaaaaaaaaaaaaaacccccccaaaaaacaaaaaaaaaccccaaaaaacccccccccccccccccccccccccccccccaaaaaa";
        public static int MapXLength { get; set; }
        public static int MapYLength { get; set; }
        public static (int Height, int X, int Y) Start { get; set; }
        public static (int Height, int X, int Y) End { get; set; }
        public static (int Height, int X, int Y)[,] ParseMap()
        {
            string[] lines = Raw.Split(Environment.NewLine);

            MapYLength = lines.Length;
            MapXLength = lines[0].Trim().Length;
            (int Height, int x, int y)[,] map = new(int Height, int x, int y)[MapXLength,MapYLength];

            for (int y = 0; y< MapYLength; y++)
            {
                string line = lines[y];
                for (int x = 0; x<MapXLength
                    ; x++)
                {
                    char c = line[x];
        ...
        public static List<(int Height, int X, int Y)> ValidStartingPoints { get; } = new();
    }
}
